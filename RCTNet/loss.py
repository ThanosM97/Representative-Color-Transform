...
class Loss():
    """Loss function as described in Kim et al. (2021). The loss function comprises
    ...
    def estimate_vgg_loss(self, Y: torch.Tensor, target_img: torch.Tensor) -> torch.Tensor:
        ...
        # for child in vgg.named_children():
        #     print(child)
        # list the names of all graph nodes (in order of execution) for vgg model

        # From these nodes we want to extract only the feature representations for
        # classification layers 2, 4, 6.
        ...
        # outputs as values
        feature_extractor = create_feature_extractor(vgg, return_nodes=return_layers)

        # transform input images to match the to input format of the pytorch pretrained
        # vgg16 model.
        Y = T.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])(Y/255)
        target_img = T.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])(target_img/255)

        # forward pass to get the feature representations of the selected vgg16 layers
        # for both the enhanced output image Y and the ground truth enhanced image
    ...
    def estimate_loss(self, Y: torch.Tensor, target_img: torch.Tensor, balance_lambda: float) -> torch.Tensor:
        ...
        """
        # Calculate the mean absolute error between the predicted and ground-truth enhanced
        #  images.
